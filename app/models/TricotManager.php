<?php
require_once('app/models/model.php');

class TricotManager extends Manager
{
    public function getTricots(){
        $bdd = $this->dbConnect();
        $req = $bdd->prepare('SELECT idTricot, title, content, img,  DATE_FORMAT(postDate, \'%d/%m/%Y\') AS dates_fr FROM tricot ORDER BY idTricot');
        $req->execute(array());
        return $req;
    }

    public function getTricot($idTricot){
        $bdd = $this->dbConnect();
        $req = $bdd->prepare('SELECT idTricot, title, content, img,  DATE_FORMAT(postDate, \'%d/%m/%Y\') AS dates_fr FROM tricot WHERE idTricot = ?');
        $req->execute(array($idTricot));
        return $req->fetch();
    }

}